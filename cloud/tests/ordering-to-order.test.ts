...
describe("buildToOrderRows", () => {
  it("keeps only rows with order > 0", () => {
    const rows: ToOrderInputRow[] = [
      { key: "a", vaccineName: "Fluad", ndc: "70461012303", group: "Flu", order: 0 },
      { key: "b", vaccineName: "Shingrix", ndc: "58160082311", group: "Other", order: 10 },
    ];
  ...
  it("uses the SAME cleaned display name every other tab shows (season + age stripped)", () => {
    ...
    const rows: ToOrderInputRow[] = [{ key: "a", vaccineName: "Fluad", ndc: "70461012303", group: "Flu", order: 20 }];
    const result = buildToOrderRows(rows);
    expect(result[0].displayName).toBe("Fluad Trivalent");
  });

  it("computes Order (pkg) from the catalog's doses-per-package, ceil-rounded", () => {
    const rows: ToOrderInputRow[] = [{ key: "a", vaccineName: "Fluad", ndc: "70461012303", group: "Flu", order: 25 }];
    const result = buildToOrderRows(rows);
  ...
  it("falls back to the researched catalog NDC when the DB row has none on file", () => {
    ...
    const rows: ToOrderInputRow[] = [{ key: "a", vaccineName: "Flucelvax PFS", ndc: null, group: "Flu", order: 5 }];
    const result = buildToOrderRows(rows);
  ...
  it("returns orderPackages: null when the catalog has no package size for the product", () => {
    const rows: ToOrderInputRow[] = [
      { key: "a", vaccineName: "Some Brand-New Vaccine", ndc: null, group: "Other", order: 8 },
    ];
    const result = buildToOrderRows(rows);
  ...
  it("groups COVID before Flu before Other, matching the main recommendation table's display order", () => {
    const rows: ToOrderInputRow[] = [
      { key: "other", vaccineName: "Shingrix", ndc: "58160082311", group: "Other", order: 5 },
      { key: "flu", vaccineName: "Fluad", ndc: "70461012303", group: "Flu", order: 5 },
      { key: "covid", vaccineName: "Some Covid Vaccine", ndc: null, group: "COVID", order: 5 },
    ];
    const result = buildToOrderRows(rows);
  ...
  it("within a group, sorts by order desc then vaccine name asc — same tie-break as the main table", () => {
    const rows: ToOrderInputRow[] = [
      { key: "low-a", vaccineName: "Aaa Vaccine", ndc: null, group: "Other", order: 3 },
      { key: "high", vaccineName: "Zzz Vaccine", ndc: null, group: "Other", order: 10 },
      { key: "low-b", vaccineName: "Bbb Vaccine", ndc: null, group: "Other", order: 3 },
    ];
    const result = buildToOrderRows(rows);
  ...
  it("returns an empty array when nothing needs ordering", () => {
    const rows: ToOrderInputRow[] = [{ key: "a", vaccineName: "Fluad", ndc: "70461012303", group: "Flu", order: 0 }];
